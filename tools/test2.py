...
def parse_args():
    ...
    parser.add_argument('--config', type=str, default='/nvme2/ROD2021/configs/config_rodnet_hg1_win16.py', help='choose rodnet model configurations')
    parser.add_argument('--data_dir', type=str, default='./data/', help='directory to the prepared data')
    parser.add_argument('--checkpoint', type=str, default='epoch_2_final.pkl',help='path to the saved trained model')
...
if __name__ == "__main__":
    ...
    for subset in seq_names:
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            output_path = os.path.join(test_res_dir, seq_name+'.txt')
            
           # if start_frame_id < 224000:
            ...
            for i in range(test_configs['test_stride']):
                ...
                write_dets_results_single_frame(res_final, cur_frame_id, output_path, dataset)
                ...
                if confmap_gt is not None:
                    ...
                    visualize_test_img(fig_name,  radar_input, confmap_pred_0, confmap_gt_0, res_final_0,
                                       dataset, sybl=sybl)
                else:
                    visualize_test_img_wo_gt(fig_name,  radar_input, confmap_pred_0, res_final_0,
                                             dataset, sybl=sybl)
            ...
            print("Load time: %.4f | Inference time: %.4f | Process time: %.4f for Seq %s at Frame %d" % (load_time, infer_time, proc_time, seq_names[0], iter))
